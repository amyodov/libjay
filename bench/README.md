...
.venv-bench/bin/python bench/workloads.py        # whole jobs, four ways
```

**Everything below measures kernels.** For whole jobs an analyst would
recognise — returns, moving-average crossovers, Bollinger bands, RSI, VWAP,
drawdown, volatility regimes, a low-pass filter, frame RMS, a single-bin
detector, a naive DFT and a rolling cross-correlation — each written in J,
in idiomatic Polars, as a numba loop and in numpy, with the source side by
side and the places libjay loses named and diagnosed, see
[workloads.md](workloads.md). That file is generated by `bench/workloads.py`
and is where to look first if the question is "what is this like to use".
