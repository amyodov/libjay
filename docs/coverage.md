...
| `":` | format: the characters that display the argument | format by specification: x is one complex `w j d` per column of y's last axis, or one for all of them — `w` the field width, `d` the digits after the point, and the rounding half-to-even for a double. A value written EXACTLY — rational or extended — breaks a tie toward the LARGER number instead, so `2 ": (1r2 1r3)` is `1 0` where `2 ": (0.5 0.333)` is `0 0`. A width of 0 takes what the column needs, with one blank in front of every column but the first; a NEGATIVE width asks for the exponential form, written from the left behind one column of sign. A value too wide for its field is written as that many asterisks rather than refused, and a character or boxed argument is a domain error |
...
| `%.` | matrix inverse — the least-squares pseudo-inverse of a taller matrix; a wider one is refused, a singular one is a domain error. A SCALAR is a reciprocal and no matrix at all, so `%. 0` is `_` where `%. (1 1 $ 0)` is refused. The EXACT types survive it wherever the answer is exact, in the rational type: a vector's pseudo-inverse is `y % (+/ y*y)` and a square matrix is Gauss-Jordan over the rationals | matrix divide: the least-squares solution of `y a = x`. A SCALAR right-hand side is the whole column of it; a VECTOR one is a column whose unknown is a single number, not a list of one; a system with NO ROWS constrains nothing and answers the zero `0 % 0` gives |
| `%.` | matrix inverse — the least-squares pseudo-inverse of a taller matrix; a wider one is refused, a singular one is a domain error. An ATOM is no matrix: it answers the plain reciprocal, exact type and all, so `%. 0` is an infinity where `%. (,0)` is still singular | matrix divide: the least-squares solution of `y a = x`. Over an ATOM y the system has one unknown, and the answer is the items' sum over that atom |
...
back to a machine number, and `_2 x: y` the OBVERSE of form 2 — `%/"1`, the
alternating quotient over the last axis, which puts back together what form
2 split apart and keeps going over a longer axis, so `_2 x: (1 2 3)` is
`1 % (2 % 3)`. The division is the ordinary one, so the exact types survive
it and a complex pair divides as complex; an axis with nothing along it
folds to the identity 1, and an axis of ONE item is that item, never
divided, which is why a lone character or box comes back untouched where a
table of them is refused. Any other left argument is a domain error, as it
is in J.
...
The EXACT types are written the same way, one offset an item and two for a
rational — the numerator and the denominator, in that order. What the
offset points at is a MAGNITUDE block: the word 227, a 2, how many bytes
the digits take, a 1, then the digit count carrying the number's sign, then
the digits themselves, sixty-four bits apiece and least significant first.
A zero has no digits and a count of zero. Measured from the reference, and
`3!:3` of both types writes its bytes exactly.
...
filled with `*`. A complex number has no format here, and the reference
refuses one too. Characters, boxes and symbols are a named gap, and round
8C measured what the reference does with them so that whoever closes it
starts from facts: a boxed argument is opened one level and each box's
contents formatted in place (`'6.2' (8!:0) (1;2)` is two boxes of `  1.00`
and `  2.00`, and a box holding another box is a domain error); a CHARACTER
argument gives one box per ROW, so the answer's shape is the argument's
without its last axis; an argument with NO ATOMS of any type answers the
same way, one box of nothing per row, which makes `'6.2' (8!:0) (0 $ 0)`
the scalar box of the empty text and `'6.2' (8!:0) (0 2 $ 0)` no boxes at
all. The specification may also arrive BOXED, one per column
(`('6.2';'8.3') (8!:0) (2 2 $ 1.5)`), and `8!:2` always answers a rank-2
array — `$ ('6.2' (8!:2) (1 2 3))` is `1 18`, where libjay answers the
rank-1 `18`.
