...
values.
## 2026-09-06 — six algorithms as examples, and how they are held to account

Six number-crunchy algorithms nobody appears to have written in J —
...
Hopfield network and a designed Savitzky-Golay filter — go in
`examples/algos/` rather than in a library, because what they demonstrate
is the notation as much as the arithmetic. docs/algorithms.md carries the
prose.

**One copy of each algorithm.** Every file is split by a line reading
`NB. === demo`: definitions above, a runnable demonstration below. The
tests, the benchmark and the corpus generator read the half above the line
straight out of the file, so nothing is transcribed and nothing can drift.
The corpus lines are generated by `python -m bench.algos.corpus` and a
pytest holds the checked-in file to what the generator produces now — a
recording is only worth what its source is.

**A shared deterministic generator rather than shared data.** libjay,
jconsole and numpy each build the same series from the same counter-based
generator; nothing carries a million literals between processes.
Multiplications modulo a prime compose into one multiplication, so the
first version was a lattice with a lag-1 autocorrelation of −0.41 and every
answer about it was an answer about the generator; two squarings fixed it.

**Tolerances are per algorithm and stated with a reason.** The metric is
the largest relative difference with a floor at a thousandth of the
answer's own scale, so a value that happens to sit near zero is judged
against the answer rather than against its own vanishing magnitude. Five of
the six agree to 1e-12 or better; the two that go through `%.` are held to
1e-8 and 1e-10 because jconsole's matrix divide behaves like a solve
through the normal equations and loses about half the digits on an
ill-conditioned system (the numbers are in docs/algorithms.md). The
extreme learning machine's corpus program therefore uses a ridge penalty
large enough to condition the system, so that what is recorded is
reproducible rather than merely typical.

**A demonstration whose answer theory already knows.** Two bugs here — J's
right-to-left `n - q + 1`, and a factor of n missing from the same
estimator in BOTH implementations — were invisible to a differential test.
