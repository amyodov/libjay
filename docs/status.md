...
| `5!:` representations | 🟡 `5!:1` the atomic representation, `5!:0` the adverb that reads one back — settled while the sentence is read, since what the representation names decides how the sentence parses — `5!:2` the boxed representation, `5!:5` the linear one and `5!:6` the parenthesised one. An explicit definition writes back as the source it was given, and its BOXED representation is a named gap, as is `5!:4`, the tree drawing |
