...
| `s:` | 🟢 symbol: a character list cut on its own leading delimiter, a character table one name per row, a boxed argument one name per box | 🟡 the name forms `2 s:` (the names razed into one list), `3 s:` and `4 s:` (a blank-padded character table) and `5 s:` (boxes). A form reads an argument with NO ITEMS whatever its type, so `5 s: (i. 0)` is the boxed empty where `5 s: 5` is a domain error, and `6 s:`, `7 s:`, `_1 s:` and `_2 s:` answer their own empty for one. `0 s:` and `1 s:` dump an interpreter's internal table, and `6 s:` … `_2 s:` number a symbol within it, so those are named over symbols; a number outside `_2` to `7` is a domain error |
| `[` | 🟢 same | 🟢 left |
| `]` | 🟢 same | 🟢 right |
| `echo` | 🟢 print | — |
