...
**J: 162 green / 15 partial / 2 absent by design, of 179 valences in the
inventory. No row in J's primitive tables is red.** The fold conjunctions
`F..` `F.:` `F:.` `F::` joined the inventory in round 5C, and `F.`/`F:` are
the one red beside them.
...
| `%.` | 🟢 matrix inverse (Householder QR, f64; a COMPLEX argument is solved as the real system of twice the size, and an argument with no elements answers the empty its inverse would have had) | 🟢 matrix divide. Its left rank is infinite, so a right-hand side of rank 3 or more is solved whole — one column per element of an item — and the answer keeps every axis but the leading one. APL's `⌹` refuses that shape, which is its own reference's rule |
| `j.` | 🟢 imaginary | 🟢 complex |
| `r.` | 🟢 angle | 🟢 polar |
...
| `p:` | 🟢 the y-th prime; extended where y is | 🟡 the prime queries: `_1` `0` `1` `2` `3` `4` `_4`. Form 3 is `q:` written the other way and reads the whole argument as `q:` does — one row per item, padded with 1s — where the rest answer about one number at a time and are framed by y's own shape. What the forms answer for a y that is NOT a whole number is 🔴 named: the reference answers some of them (`0 p: 2.5` is 1, `4 p: 1.5` is 2) and refuses others (`2 p: 1.5`), by no rule black-box probing could settle |
...
| `{.` | 🟢 head | 🟢 take |
| `}.` | 🟢 behead; an ATOM is one item, so beheading it is the empty list of the atom's own type | 🟢 drop |
| `{:` | 🟢 tail | — |
| `}:` | 🟢 curtail | — |
...
| `{::` | 🟢 map | 🟢 fetch: the left rank is 1, so a path of higher rank is a FRAME of paths — `(2 1 $ 0 1) {:: ((1 2);(3 4))` is the two boxes opened and stacked, and a frame with no cell is the empty the frame spells |
| `i.` | 🟢 integers | 🟢 index of |
...
| `L.` | 🟢 level of | — |
| `":` | 🟢 default format | 🟢 format by specification: `w j d` per column, width 0 for what the values need, a negative width for the exponential form, asterisks for what does not fit. A COMPLEX specification is that `wjd` written as one number, and a fractional part in either half is refused |
...
| `s:` | 🟢 symbol, which is `_5 s:`, `_4 s:` or `_1 s:` chosen by what it is handed: a boxed argument one name per box, a character array of rank 2 or more one name per row with the trailing blanks trimmed, a character list cut on its own leading delimiter | 🟡 the ten forms that are a function of the argument, in inverse pairs: `1`/`_1` raze the names with a backquote BEFORE each, `2`/`_2` with a null AFTER each, `3`/`_3` lay them out as a character table padded with NULLS, `4`/`_4` padded with BLANKS, and `5`/`_5` box them one apiece. A forward form reads an argument with NO ITEMS whatever its type, so `5 s: (i. 0)` is the boxed empty where `5 s: 5` is a domain error; a negative form READS text and refuses a kind that is not text at any size. The remaining four — `0 s:` with its numbered queries, `6 s:` and `_6 s:` for the SLOT a name was interned into, `7 s:` for the ORDER it was interned in — report an interpreter's own table and are 🔴 named as belonging to no language, except over an argument with nothing to number. A number outside `_6` to `7` is a domain error |
| `[` | 🟢 same | 🟢 left |
| `]` | 🟢 same | 🟢 right |
| `echo` | 🟢 print | — |
...
| `/` | 🟢 insert | 🟢 table |
| `\` | 🟢 prefix | 🟢 infix |
| `\.` | 🟢 suffix; a GERUND over an argument with no items answers the boxed empty, where the prefixes take the shape a run would have made | 🟢 outfix; a piece of one item applies nothing, and only the five folds J has special code for (`+/` `*/` `<./` `>./` `+./`) type the whole argument first |
| `/.` | 🟢 oblique | 🟢 key |
| `~` | 🟢 reflex | 🟢 passive |
...
| `@` atop | 🟢 |
| `@:` at | 🟢 |
| `&` bond / compose | 🟢 verbs compose and nouns bond, a noun the program COMPUTES as readily as a literal one — `n =: 3` and then `n & +` reads the name where the derived verb is applied, as a `^:` count does |
| `&:` appose | 🟢 |
...
| `^:` power | 🟢 a count the program COMPUTES as readily as a literal one — a name, an expression, a definition's own argument — read where the derived verb is applied and not while the program compiles; a list of counts (mixed signs included), `_`, the traces `u^:(<n)` and `u^:a:`, a verb count — which means by what it computes what the same count written as a literal means, negatives and lists included — and negatives — which obverse a negative count runs is settled when the arguments arrive, so `x u^:_1 y` undoes the bond `x&u` and the monad undoes u |
...
| `!:` foreign | 🟡 the members that only COMPUTE, family by family: `1!:1` (read a line from stdin), `1!:2` (write a line to stdout); `3!:0` (type code), `3!:1` `3!:2` `3!:3` (the binary form, out and back and as hexadecimal), `3!:4` `3!:5` (numbers to bytes and back); `4!:0` `4!:1` `4!:55` (the class a name has, the names of a class, erase); `5!:0` `5!:1` `5!:2` `5!:5` `5!:6` (the representations and the adverb that reads one back); `8!:0` `8!:1` `8!:2` (the formats); `9!:10` `9!:11` `9!:18` `9!:19` (print precision and comparison tolerance, both HONOURED by what comes after them); `128!:3` (the crc); and the locale family `18!:0 18!:1 18!:2 18!:3 18!:5 18!:55`. The ones that reach a file, a script, the host, the clock or a shared library are ⚪ closed by the sandbox; `7!:` (the interpreter's own memory), `13!:` (its debugger), the rest of `9!:` (its settings), `18!:4` and `18!:6` are ⚪ machinery a second implementation has no counterpart for. 🔴 named: the binary form of the exact types (`1r2`, `1x`), the `8!:` format of characters and boxes, `5!:4` (the tree drawing), `9!:0` `9!:1` (the random seed) and the `128!:` numerical members — `128!:0` QR, `128!:1`, `128!:10` and the rest |
| `` ` `` tie (gerund) | 🟡 the gerund is boxed data — one atomic representation per box, so it can be named, computed and displayed, an explicit definition and a capped fork included; a verb the representation has no words for is named. As an ADVERB'S operand it cycles: `` u`v/ `` inserts the verbs between the items and `` u`v\ ``, `` u`v\. `` and `` u`v/. `` give one verb to each prefix, suffix, group, window or diagonal in turn, dyadic infix and outfix included; under any other adverb it is still named. Two CONJUNCTIONS hand it out per piece too — the cut `` u`v;.n `` one verb per piece except the TESSELATING one, which takes none, and `` u`v"n `` one per cell of the rank's frame, where a single box or a rank infinite in all three places is the constant verb `m"n` instead and the dyad has no meaning |
| `` `: `` evoke gerund | 🟢 the three forms J gives it: `0` applies each verb, `3` inserts them, `6` is the train |
| `@.` agenda | 🟢 the index is read the way `{` reads one, so a negative counts back from the end of the gerund, and a LIST of indices is the train the verbs it picks spell — one verb, a hook or a fork, where a fourth has no meaning even though `` `:6 `` gives one |
| `[:` cap | 🟢 |
| `::` adverse | 🟢 |
...
| `b.` boolean / characteristics | 🟢 `m b.` (the 32 boolean and bitwise functions, and the three that move bits: 32 rotates y left by x places, 33 shifts it logically, 34 arithmetically), its one-argument form `(m b.) y` — the same table read with a left argument of zero — and all three characteristics: `0` the ranks — a NEGATIVE rank leaves a fixed number of frame axes and so has no bound, and is reported as infinite — `1` the identity function, `_1` the obverse. `_1` answers a SPELLING, and libjay writes its own — a derived verb whose operand is a noun spells as `(n&+)` here where the reference writes the noun out |
...
| `H.` hypergeometric | 🟢 both valences: the monad sums the series to its limit, with the shared parameters cancelled, and `x (m H. n) y` stops after x terms — a whole nonnegative count, paired with the argument element by element. A term count of ZERO reads no argument at all, so a character argument answers a zero per element rather than a refusal. A series that neither converges nor overflows is refused by name, and one whose running total becomes a NaN of the arithmetic's own making is refused as the reference refuses it — `3 (2 H. 2) _` is `_` where `3 (2 H. 2) __` and `3 (2 H. 2) (1j_)` have no value. A parameter list written out rather than typed in (`(i. 0) H. 2`) is folded where the sentence is parsed |
| `F..` `F.:` `F:.` `F::` fold | 🟢 v folds the items of y into a running value — the ITEM on v's left, the running value on its right — and u is applied to each running value to make the answer. The first inflection says whether the answer is the last running value alone or every one of them framed; the second says whether the items are taken from the front or from the back. A left argument is where the running value starts; with none, the first item in the direction of travel is, and with no item either the running value is what an insert of v over the empty answers — which the MULTIPLE forms, having no step to report, refuse instead. `F.` and `F:`, the two spellings whose number of steps a test decides rather than the argument, are 🔴 named: each is unbounded, and the reference runs without end on the ordinary cases of them |
...
| Verb (tacit) assignment `mean =. +/ % #` | 🟡 a sentence of its own. The reference reads an assignment as a VALUE too, so `2 [ f =. #` is the noun fork the assigned verb makes; libjay settles what a name stands for while the sentence is parsed and names that a gap |
| Displaying a bare tacit-verb name (`mean` after `mean =. +/ % #`) | 🟡 the linear representation, bracketed where the spelling would otherwise read as something else; a cap fork gives back the `[: f g` it was written as, and `u"b a b` is written `u"a b`, the shorter of two spellings of one rank |
| Displaying a bare modifier name (`m` after `m =. /`) | 🟢 a primitive modifier, and an explicit one however it was written — inline, on the lines below, or between a pair of braces |
| `explain` facility | 🟢 |
...
| Tacit definition `13 : '…'` | 🟢 the translation, what it computes, and the `[: f g` the reference writes wherever the translation composes monadically; a body the abstraction cannot reach becomes the explicit definition, as the reference's own fallback does |
...
| `4!:` the name table | 🟢 `4!:0` the class a name has (0 noun, 1 adverb, 2 conjunction, 3 verb, `_1` no meaning, `_2` no name), `4!:1` the names of the classes asked for, sorted, and `4!:55` erase. A foreign that takes boxed names, asked about NO name at all, answers an empty rather than reading the empty argument's type. A name has one class at a time: naming a verb takes away the value it held |
| `5!:` representations | 🟡 `5!:1` the atomic representation, `5!:0` the adverb that reads one back — settled while the sentence is read, since what the representation names decides how the sentence parses — `5!:2` the boxed representation, `5!:5` the linear one and `5!:6` the parenthesised one. An explicit definition is the `:` conjunction over its valence and its body in all four, whichever way the source spelled it, and a name of modifier class answers for the modifier it was given. `5!:4`, the tree drawing, is a named gap |
| `8!:` formats | 🟡 `8!:0` a box an atom, `8!:1` a box a column, `8!:2` a character array, each with the C spelling of a number rather than J's (`-1.5`, not `_1.5`) and columns of one width; a literal `width.decimals` on the left sets the field, and a width too narrow is filled with `*`. Characters, boxes and symbols are a named gap; a complex number has no format here, as the reference answers |
| `9!:` global parameters | 🟡 `9!:10`/`9!:11` the print precision and `9!:18`/`9!:19` the comparison tolerance, both read and written, and a setting takes effect on every sentence AFTER it in the same program. The rest of the family is the interpreter's own machinery and is ⚪; the random seed `9!:0`/`9!:1` is 🔴 named |
| `128!:` numerical foreigns | 🟡 `128!:3`, the CRC-32 of a byte string as a signed 32-bit number. The decompositions — `128!:0` QR and the rest — are 🔴 named |
